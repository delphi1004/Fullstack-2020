  ...
  return (
    <div>
      <div style={hidenWhenVisible}>
        <button id='toogleButton' style={{ backgroundColor: 'lightblue', border: 'none' }} onClick={toggleVisibility}>{props.buttonLabel}</button>
      </div>
      <div style={showWhenVisible}>
        {props.children}
        <button style={{ width: 60 }} onClick={toggleVisibility}>cancel</button>
      </div>
    </div>
  )
})
