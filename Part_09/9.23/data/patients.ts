import { Patient, Gender, EntryType, HealthCheckEntry, HospitalEntry, BaseEntry, HealthCheckRating, OccupationalHealthCareEntry } from '../types';
...
const parseOccupationalHealthcare = (entry: OccupationalHealthCareEntry) => {
  ...
  if (entry.sickLeave !== undefined) {
    if (!entry.sickLeave.startDate || !isDate(entry.sickLeave.startDate)) {
      throw new Error(`Incorrect or missing patient's start date of sick leave: ${entry.sickLeave.startDate}`);
    }

    if (!entry.sickLeave.endDate || !isDate(entry.sickLeave.endDate)) {
      throw new Error(`Incorrect or missing patient's end date of sick leave: ${entry.sickLeave.endDate}`);
    }
  }
};

const parseEntryFieldType = (patientEntry: Array<EntryType>): Array<EntryType> => {

  if (patientEntry) {
    patientEntry.forEach(entry => {
      if (!entry.type || (entry.type !== 'HealthCheck' && entry.type !== 'OccupationalHealthcare' &&
        entry.type !== 'Hospital')) {
        throw new Error('Incorrect or missing entry type');
      } else {
        switch (entry.type) {
          case "HealthCheck": parseHealthCheckEntry(entry); break;
          case "OccupationalHealthcare": parseOccupationalHealthcare(entry);break;
          case "Hospital": parseHospitalCheckEntry(entry); break;;
        }
      }
    });
  }
...
export const toNewPatientEntry = (entries:any): Array<EntryType> => {
